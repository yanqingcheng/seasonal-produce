...
This repository is the seasonal produce calendar for idea #22. The United Kingdom shipped first. France, Spain, Ontario, Italy, and Florida ship on the same wheel. The work follows the museum-ship pipeline.
...
[ARCH.md](ARCH.md) is the Stage 4 high-level design. The signed-off sticker-garden wheel is the shared graphics layer. Stage 5 shipped the UK pack on that wheel. A later country is another pack plus a selector, not a new wheel. France, Spain, Ontario, Italy, and Florida are those packs. There is still no globe. Where ARCH.md and DESIGN.md disagree on the UK clock, the UK staple ranking, or which UK items may be stickers, ARCH.md wins. Motion and art stay in DESIGN.md.

## Thin ship

Stage 5 publishes one static exhibit in [`site/`](site/). The wheel is shared. Shipped packs are the United Kingdom, France, Spain, Ontario (`on`, never a Canada-wide pack), Italy (`it`), and Florida (`fl`, never a United States pack). A place selector appears when the registry lists more than one shipped pack.
...
Open `http://localhost:8000/`. With no query, the wheel lands on the current month in the pack timezone (Europe/London for the UK). `?month=9` opens September. `?item=apple` opens that fact sheet after the arrival spin. `?country=fr`, `?country=es`, `?country=on`, `?country=it`, and `?country=fl` load those packs. Any other `?country=` value, including `us`, falls back to the UK pack.
